...
.requestMatchers("/api/v1/rentals/**")
...
\begin{lstlisting}[language=bash, caption={application.properties --- prima e dopo}]
# Prima (vulnerabile)
spring.datasource.password=andrea72
...
\begin{lstlisting}[language=Java, caption={RateLimitFilter.java --- logica token-bucket}]
...
\begin{itemize}
    ...
    \item \textbf{V09}: access token 1 ora (\texttt{expiration=3600000}), refresh token 7 giorni (\texttt{refresh-expiration=604800000}).
    ...
    \item \textbf{V13}: path Swagger spostati sotto \texttt{.authenticated()}.
...
del progetto RentACar. La suite comprende \textbf{220 casi di test} organizzati in
8 macrogruppi tematici, ciascuno focalizzato su una specifica classe di vulnerabilità
conforme al framework OWASP Top 10 2021.

\textbf{Stato finale (2026-04-12):}
\begin{itemize}
    \item Totale test: 220
    \item Pass: 220 (100\%)
    \item Fail: 0
    \item Skipped: 0
\end{itemize}
...
\texttt{AuthenticationControllerSecurityTest} & Validazione input auth, error handling \\
\texttt{CorsSecurityTest} & CORS misconfiguration, CSP header, wildcard \\
\texttt{CorsAttackerDomainSecurityTest} & Domini ostili in CORS whitelist \\
\texttt{CorsConfigDuplicationSecurityTest} & CORS duplicata (V12) \\
\texttt{EmployeeControllerSecurityTest} & ACL su /api/v1/employees/** \\
\texttt{FileUploadSecurityTest} & Validazione tipo file upload (V07) \\
\texttt{GenericExceptionHandlerSecurityTest} & Exception disclosure (V08) \\
\texttt{HardcodedCredentialsSecurityTest} & Credenziali hardcoded (V04) \\
\texttt{InputValidationSecurityTest} & Validazione query param, payload JSON \\
...
\texttt{SwaggerProdAccessSecurityTest} & Swagger UI protection (V13) \\
\texttt{UserControllerSecurityTest} & ACL su /api/v1/users/**, IDOR \\
\hline
\end{tabular}
\caption{Classi di test di sicurezza (22 classi nel package \texttt{security/})}
...
I 220 test sono organizzati in 8 macrogruppi tematici:
...
\textbf{Scope:} 37 test | \textbf{OWASP:} A03, A04 | \textbf{CWE:} CWE-20, CWE-434
...
\begin{itemize}
    ...
    \item \texttt{FileUploadSecurityTest} (4 test — protezione incidentale)
\end{itemize}
...
\textbf{Stato:} \checkmark{} \textbf{37/37 test PASS (100\%)} | 0 FAIL
...
Classi di test:
\begin{itemize}
    \item \texttt{FileUploadSecurityTest} (4 test — copertura parziale)
    \item \textbf{MANCANTI:} PaymentDetailControllerSecurityTest
...
CWE-434 & Unrestricted File Upload & FileUpload & V07 \\
...
Test di regressione aggiunti & 22 \\
\hline
Coverage OWASP A\# & 6 categorie completamente \\
Coverage CWE & 16 CWE mappate \\
\hline
\end{tabular}
\caption{Statistiche Suite di Sicurezza (2026-04-12)}
...
\hline
\textbf{Macrogruppo} & \textbf{OWASP} & \textbf{Test} & \textbf{Pass} & \textbf{Fail} \\
\hline
Broken Access Control & A01 & 87 & 87 & 0 \\
Security Misconfiguration & A05 & 18 & 18 & 0 \\
Cryptographic Failures & A02 & 5 & 5 & 0 \\
Authentication Failures & A07 & 17 & 17 & 0 \\
Logging Failures & A09 & 5 & 5 & 0 \\
Injection \& Input Validation & A03/A04 & 37 & 37 & 0 \\
JWT \& Token Management & A07 & 30 & 30 & 0 \\
Application-specific & Vario & 4 & 4 & 0 \\
\hline
\end{tabular}
\caption{Distribuzione test per macrogruppo (alcune classi coprono più categorie; totale suite: 220 test)}
...
\textbf{Descrizione:} 6 credenziali committate in chiaro nel repository:
\begin{itemize}
    \item \texttt{spring.datasource.password=andrea72}
    \item \texttt{jwt.secret-key=404E635266556A586E...} (32 byte hex)
    \item \texttt{cloudinary.api-key=752662656464259}
    \item \texttt{cloudinary.api-secret=J1HXxfU3tc2VP2sjN1hYoauxEZ0}
    \item \texttt{spring.mail.password=vsnvlkqlvtdvmwdh}
\end{itemize}
...
\textbf{Patch applicata:}
\begin{enumerate}
    \item Sostituite tutte con placeholder: \texttt{\$\{DB\_PASSWORD\}}, \texttt{\$\{JWT\_SECRET\_KEY\}}, ecc.
    \item Aggiunto \texttt{application-test.properties} con valori fittizi
    \item \textbf{AZIONE OPERATIVA:} Ruotare tutte e 6 le credenziali nei sistemi reali
\end{enumerate}

\textbf{Test di regressione:}
\begin{itemize}
    \item \texttt{HardcodedCredentialsSecurityTest.applicationProperties\_mustNotContainKnownDbPassword()} → \checkmark{} PASS
    \item \texttt{HardcodedCredentialsSecurityTest.applicationProperties\_mustNotContainKnownCloudinarySecret()} → \checkmark{} PASS
\end{itemize}

\textbf{Impact:} Alta (credential exposure in git history) → Post-patch: Mitigata (env vars)
...
\begin{enumerate}
    ...
    \item Access token: \texttt{expiration = 3600000} (1 ora)
...
\begin{enumerate}
    ...
    \item Spostati sotto \texttt{.authenticated()} nella catena di sicurezza
    \item Disabilitati completamente su profilo \texttt{prod}
\end{enumerate}
...
\hline
2 & 2026-04-12 & ALTA & \checkmark{} COMPLETATA: V06 rate limiting (Bucket4j) \\
  &  &  & \checkmark{} COMPLETATA: V10 token revocation (RefreshTokenEntity) \\
  &  &  & \textit{(in corso)} V07 magic bytes validation (Tika) \\
  &  &  & \textit{(in corso)} Payment/Customer controller test (+35 test) \\
\hline
3 & 2026-07-01 & MEDIA & \textit{(in corso)} Discount/Vehicle controller test (+20 test) \\
  &  &  & \textit{(in corso)} Mutation testing (PIT) \\
\hline
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metrica} & \textbf{Attuale (2026-04-11)} & \textbf{Target (Post 1.3)} \\
\hline
Pass rate & 100\% (220/220) & 100\% \\
OWASP coverage & 5/10 completamente, 2 parziali & 7/10 completamente \\
Vulnerabilità patchate & 11/13 & 13/13 \\
Test per endpoint critico & 2–3 & 5+ \\
Mutation score & TBD & $\geq$ 75\% \\
Coverage endpoint payment/customer & 4 test & 50+ test \\
\hline
\end{tabular}
...
\section{Containerizzazione dell'Applicazione con Docker}
  ...
  \begin{itemize}
      ...
      directory \texttt{secrets/} (non versionata in git). Docker Compose monta questi
      file come read-only in \texttt{/run/secrets/} all'interno dei container che li
      dichiarano. Questo è il meccanismo consigliato per gestire credenziali in
...
7 nuovi file di produzione, modificati oltre 10 file esistenti e rimosse le 6
credenziali hardcoded dalla configurazione.

La verifica è stata condotta tramite una suite di \textbf{220 test di sicurezza},
organizzati in 22 classi tematiche e strutturati su quattro livelli (unit,
web-layer, integration, negative testing). Il tasso di successo finale è del
\textbf{100\%} (220/220), con copertura di 6 categorie OWASP Top 10 2021 e
16 CWE mappate. La suite è integrata nella pipeline Maven con gate JaCoCo
obbligatori (line coverage $\geq 60\%$, class coverage $\geq 70\%$).
